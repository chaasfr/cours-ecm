...
<%@ taglib prefix="myfn" uri="/WEB-INF/tld/function.tld" %>
...
<div class="row">
    ...
    <div class="col-xs-12 col-sm-8">
        ...
        <c:forEach var="i" items="${fn:escapeXml(recipe.tags)}">
            <span class="label label-primary">${i.replace("[","").replace("]","")}</span>
        </c:forEach>
        ...
        <p>${myfn:textFormating(recipe.text)}</p>
    </div>
</div>